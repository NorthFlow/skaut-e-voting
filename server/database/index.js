...
userdb.getAnswerCountPerQuestion = (voting_id) => {
    return new Promise((resolve, reject) => {
        connection.query(`SELECT id_question, COUNT(id_answer) AS acount 
            FROM answers join questions using(id_question)
            WHERE questions.id_voting=? GROUP BY (id_question)`, [voting_id], (err, result) => {
            if (err) {
                return reject(err);
            }
            return resolve(result);
...
userdb.getQaAInVoting = (voting_id) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        FROM answers JOIN questions ON (questions.id_question=answers.id_question) WHERE questions.id_voting=?
